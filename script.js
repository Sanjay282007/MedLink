// MedLink - script.js (complete updated version)
...
// --- OneSignal deferred helper (in case OneSignal loads after our script) ---
...
window.toggleAuthMode = () => {
  ...
  if (!authTitle || !submitButton || !toggleAuthButton || !registrationFields || !usernameInput) return;
...
async function scheduleMedicineReminder(medicineName, time) {
  try {
    // Wait a short time to let OneSignal (if present) initialize
    await new Promise(r => setTimeout(r, 1200));

    // Try to obtain player id using deferred queue if present
    const playerId = await new Promise((resolve) => {
      // If OneSignal is ready on window
      ...
      // Otherwise push into OneSignalDeferred and let loader call queued functions when ready
      ...
      // Fallback: resolve null after 2s if nothing happens
    ...
    // Send medicine data to Pipedream endpoint
    ...
    // attempt to parse JSON; some endpoints may return text
    let result;
    try { result = await response.json(); } catch(e){ result = await response.text(); }

    console.log("✅ Reminder scheduled (Pipedream response):", result);
  } catch (err) {
...
const showMainView = (viewId) => {
  ...
  if (viewId === 'auth-view' && authView) authView.classList.remove('hidden');
  else if (viewId === 'app-view' && appView) {
...
const prepareForm = (medicineData) => {
  ...
  if (!form || !formTitle || !saveButton) return;

  form.reset();
  currentEditingId = null;
  if (scheduleTimeInput) scheduleTimeInput.value = '';
  if (doseTimeSelect) doseTimeSelect.value = '';
...
window.saveMedicine = () => {
  ...
  // schedule a remote reminder for this medicine (if notifications available)
  try {
    scheduleMedicineReminder(newMedicine.name, newMedicine.time);
  } catch(e) {
  ...
  if (ft) ft.textContent = 'Add New Medicine';
  if (sb) sb.textContent = 'Save Medicine';
  if (scheduleTimeInput) scheduleTimeInput.disabled = true;
...
window.takeDose = (id) => {
  ...
  if (medicine.stock >= medicine.doseAmount) {
    ...
    // schedule remote reminder (re-affirm next occurrence)
    try { scheduleMedicineReminder(medicine.name, medicine.time); } catch(e){ console.warn(e); }
...
window.renderDashboard = () => {
  ...
  if (summaryButton) summaryButton.disabled = medicines.length === 0;
  ...
  if (statTotal && statUrgent && noMedsMessage) {
    ...
    if (medicines.length === 0 && (searchInput && (searchInput.value||'') === '')) {
...
function renderSummaryCards() {
  if (!lowStockListEl || !expiringSoonListEl || !lowStockThresholdText || !expiryWarningDaysText || !todayDateText) return;
...
function updatePermissionStatus(){
  const statusText = {
    'granted':'✅ Permission Granted: Browser notifications are available.',
    'denied':'🚫 Permission Denied: Browser notifications are blocked.',
    'default':'❓ Permission Required: Click the button to enable browser notifications.'
  }[notificationPermission];
  if (!permissionStatus || !permissionButton) return;
...
const showReminderModal = (med) => {
  ...
  activeReminder = med;
  const rn = document.getElementById('reminder-name');
  const rd = document.getElementById('reminder-details');
  const rp = document.getElementById('reminder-purpose');
  if (rn) rn.textContent = med.name;
  if (rd) rd.textContent = `Dose: ${med.dosage} (Take ${med.doseAmount} unit${med.doseAmount>1?'s':''})  •  Schedule: ${med.scheduleType === 'daily' ? 'Daily' : 'Today'} @ ${med.time}`;
  if (rp) rp.textContent = med.purpose || "No specific notes.";
  if (reminderModal) { reminderModal.classList.remove('hidden'); reminderModal.classList.add('flex'); }
  ...
  try {
    if (alarmSound) { alarmSound.loop = true; alarmSound.volume = 1.0; alarmSound.currentTime = 0; alarmSound.play().catch(()=>{}); }
  } catch(e){}
};
...
window.closeReminder = () => {
  ...
  activeReminder = null;
  if (reminderModal) { reminderModal.classList.add('hidden'); reminderModal.classList.remove('flex'); }
  stopAlarm();
  try { if (notificationSound) { notificationSound.pause(); notificationSound.currentTime = 0; } } catch(e){}
  renderDashboard();
  renderSummaryCards();
};
...
const initializeDarkMode = () => {
  ...
  if (darkModeToggle) darkModeToggle.innerHTML = isDark ? '🌙' : '☀';
};
if (darkModeToggle) {
